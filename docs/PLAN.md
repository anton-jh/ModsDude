...
- [ ] **Regenerate `Generated.cs`** — it predates `ProblemType.RepoNotEmpty`, so the client
      cannot parse that problem. Needs a running API; see
...
- [ ] The two API changes that make the above usable, neither of which exists:
      `RegisterModRequest` grows a `Locked` field, and a `PUT repos/{repoId}/mods/{modId}`
      (Member) lets the user change it afterwards. There is no endpoint that mutates a `Mod`
      today, so "the user changes it later from the relevant views" currently has nothing to
      call.
