...
| `SizeBytes` | How big the stored file is, read from storage at registration rather than taken from the client, which cannot be checked. Required: registration refuses a file storage cannot give a size for |
