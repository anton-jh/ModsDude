...
public class ProfileApplyTargetsTests
{
    ...
    [Fact]
    public void The_targets_are_the_games_already_on_that_profile()
    {
        var mine = Game("Farming Simulator 25", new ActiveProfile(_repoId, _profileId));
        var other = Game("Dedicated server", new ActiveProfile(_repoId, Guid.NewGuid()));
        var none = Game("Fresh install", null);
    ...
    [Fact]
    public void A_drifted_game_is_in_the_set_by_definition_and_needs_no_pre_selection()
    {
        // Drift is a folder no longer matching its own active profile, so nothing about it changes
        // which games a re-apply targets.
        var drifted = Game("Farming Simulator 25", new ActiveProfile(_repoId, _profileId));
    ...
    {
        var elsewhere = Game("Farming Simulator 25", new ActiveProfile(Guid.NewGuid(), _profileId));

        Assert.Empty(ProfileApplyTargets.Derive([elsewhere], new ActiveProfile(_repoId, _profileId)));
    }

    [Fact]
    public void Several_games_on_one_profile_are_all_targets()
    {
        var active = new ActiveProfile(_repoId, _profileId);
        var singleplayer = Game("Singleplayer", active);
        var server = Game("Dedicated server", active);

        Assert.Equal([singleplayer, server], ProfileApplyTargets.Derive([singleplayer, server], active));
    }

    [Theory]
    // One game shows nothing at all - the word "game" never appears, which is the common
    // case for most games. Zero drops the apply entirely.
    [InlineData(0, "Save changes")]
    [InlineData(1, "Save and apply")]
    [InlineData(3, "Save and apply to 3 games")]
