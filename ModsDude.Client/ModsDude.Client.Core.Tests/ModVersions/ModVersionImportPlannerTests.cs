...
public class ModVersionImportPlannerTests
{
    ...
    public void A_mod_the_comparer_settled_is_ready_without_waiting_on_the_ambiguous_ones()
    {
        ...
        Assert.Equal("FS25_Plain", Assert.Single(plan.Ready).ModId);
        Assert.Equal("FS25_Awkward", Assert.Single(plan.Arbitration).ModId);
    ...
    {
        var plan = ModVersionImportPlanner.Plan([_unambiguous, _ambiguous, _unambiguous with { ModId = "FS25_Other" }], _comparer);

        Assert.Equal(["FS25_Awkward"], plan.ModIdsSkippedByCancelling);
        Assert.Equal(["FS25_Plain", "FS25_Other"], plan.Ready.Select(x => x.ModId));
    }

    [Fact]
    public void An_arbitration_item_lists_the_versions_in_the_order_that_was_derived()
    {
        var candidate = new ModVersionImportCandidate("FS25_Awkward", ["1.0", "2.0"], ["v1", "1.5"]);

        var item = Assert.Single(ModVersionImportPlanner.Plan([candidate], _comparer).Arbitration);

        Assert.Equal(["1.0", "v1", "1.5", "2.0"], item.Versions.Select(x => x.VersionId));
    }
    ...
    public void An_arbitration_item_marks_the_versions_nothing_could_place()
    {
        ...
        Assert.Equal(["v1"], item.Versions.Where(x => x.IsUnplaceable).Select(x => x.VersionId));
        Assert.Equal(new ModVersionPair("1.0", "v1"), Assert.Single(item.UnorderedPairs));
    }
    ...
    public void An_arbitration_item_separates_the_incoming_versions_from_the_registered_ones()
    {
        ...
        Assert.Equal(["1.0", "2.0"], item.RegisteredInOrder);
        Assert.Equal(["v1", "1.5"], item.Incoming);
    }
    ...
    public void An_arbitrated_item_turns_back_into_placements()
    {
        ...
        var resolved = ModVersionPlacementPlanner.PlanFor(item.ModId, item.RegisteredInOrder, item.Incoming, ["v1", "1.0", "1.5", "2.0"]);

        Assert.False(resolved.NeedsArbitration);
        Assert.Equal(
            [
                new ModVersionRegistration("v1", new ModVersionPlacement(null, "1.0")),
                new ModVersionRegistration("1.5", new ModVersionPlacement("1.0", "2.0"))
            ],
