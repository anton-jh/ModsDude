...
/// <summary>
/// Points a profile back at one of its past savegames, so that savegame follows the mod list again.
/// </summary>
/// <remarks>
/// <para>
...
/// savegame on its own: a profile with no current savegame is a state reached by deleting the current
...
/// says nothing about which savegame a profile follows. Refusing over it would also make the archived
/// incumbent unreplaceable, which is one of the three ways out this state is meant to have.
/// </para>
/// <para>
/// Member, like publishing and checking in. It supersedes somebody's savegame rather than destroying
...
public class MakeSavegameCurrentV1Endpoint : IEndpoint
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // way to give a savegame a profile is to republish it; see docs/10-savegame-profile-binding.md.
        ...
        // following this savegame, and it is - and a second client that had not caught up yet is exactly
        // who lands here.
        ...
        catch (DbUpdateException exception) when (SavegameConflicts.IsCurrentSavegameConflict(exception))
        {
            ...
            // same request made for another of the profile's past savegames. The index is what decides;
    ...
    /// The savegame this one displaced, or <c>null</c> where the profile had none - which is the state a
