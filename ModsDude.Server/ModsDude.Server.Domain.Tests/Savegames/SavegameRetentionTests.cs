...
public class SavegameRetentionTests
{
    ...
    {
        Assert.Empty(SavegameRetention.PlanPrune([], SavegameSnapshotNumber.None));
    }
    ...
    [Fact]
    public void The_most_recent_snapshots_are_the_ones_kept()
    {
    ...
    /// </summary>
    [Fact]
    public void The_plan_is_oldest_first_whatever_order_the_snapshots_arrived_in()
    {
    ...
    /// <summary>
    /// The head is not always the highest number - a savegame restored onto an older snapshot, or one
    /// whose newest snapshots were pruned by a policy that has since been widened, has a head inside
    ...
    /// <summary>
    /// Labelling a snapshot is the gesture by which a person says to keep it - a rule small enough to
    /// hold in your head, and one that reuses a field the history already has.
    /// </summary>
    [Fact]
    public void A_labelled_snapshot_is_never_pruned()
    {
    ...
    /// <summary>
    /// The deliberate half of the rule: labelled snapshots are <b>exempt</b> from pruning rather than
    /// <b>counted</b> against the limit. Ten kept snapshots from last spring must not silently push
    ...
    /// </summary>
    [Fact]
    public void Labelling_old_snapshots_does_not_push_recent_ones_out_of_the_kept_set()
    {
        var snapshots = new[]
        {
        ...
        var plan = SavegameRetention.PlanPrune(snapshots, Head(15), keep: 5);

        // Under a counted reading the ten labelled ones would consume the whole allowance and
        // snapshots 11 to 14 would go, which is the opposite of what the limit is for.
        Assert.Empty(plan);
    }
    ...
    [Fact]
    public void The_default_keeps_ten_snapshots()
    {
        var snapshots = Enumerable.Range(1, 12).Select(Unlabelled).ToList();

        var plan = SavegameRetention.PlanPrune(snapshots, Head(12));

        Assert.Equal(SavegameRetention.DefaultSnapshotsKept, 12 - plan.Count);
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public void A_savegame_cannot_be_kept_to_fewer_than_one_snapshot(int keep)
    {
    ...
    /// <summary>
    /// The recency window counts unlabelled snapshots only. A labelled one is kept whatever happens,
    ...
    /// the loss. Snapshots 4 and 5 are named here, so the two the window is for are 3 and 2.
    /// </summary>
    [Fact]
    ...
    private static SavegameSnapshotNumber Number(int value) => new(value);
    private static SavegameSnapshotNumber Head(int value) => new(value);

    private static SavegameSnapshotRetention Unlabelled(int number) => new(Number(number), false);
    private static SavegameSnapshotRetention Labelled(int number) => new(Number(number), true);

    private static IReadOnlyList<SavegameSnapshotRetention> Unlabelled(params int[] numbers)
